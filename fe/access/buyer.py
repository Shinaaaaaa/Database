...
class Buyer:
    ...
    def send_out_delivery(self, seller_id, store_id, order_id):
        json = {"user_id": seller_id, "store_id": store_id, "order_id": order_id}
        url = urljoin(self.url_prefix, "send_out_delivery")
        headers = {"token": self.token}
        r = requests.post(url, headers=headers, json=json)
        return r.status_code
    
    def take_delivery(self, order_id):
        json = {"user_id": self.user_id, "token": self.token, "order_id": order_id}
    ...
    def order_query(self):
        ...
        url = urljoin(self.url_prefix, "order_query")
        headers = {"token": self.token}
        r = requests.post(url, headers=headers, json=json)
